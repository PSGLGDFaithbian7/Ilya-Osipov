...
# 切换方式：在 io.lst 里写一行  InternalFlow : 1
# -----------------------------------------------------------------------------
...
# ------------------------ 流程选择：先看 io.lst 里有没有 InternalFlow : 1 ------------------------
set INTERNAL_FLOW 0
# 把 io.lst 先扫一遍拿开关
seek $fh_io 0
while {[gets $fh_io raw] >= 0} {
    set line [string trim [lindex [split $raw "#"] 0]]
    if {[regexp -nocase {^InternalFlow\s*:\s*([01])} $line -> INTERNAL_FLOW]} {
        break
    }
}
seek $fh_io 0        ;# 回绕，后面正式再读一遍
...
proc source_io_flow {fh_out} {
    ...
    upvar #0 DEFAULT_WIRE_LOAD_MODE DEFAULT_WIRE_LOAD_MODE
    upvar #0 DEFAULT_WIRE_LOAD_SELECTION DEFAULT_WIRE_LOAD_SELECTION
    upvar #0 ENABLE_SET_OPERATING_CONDITIONS ENABLE_SET_OPERATING_CONDITIONS
