# scripts/signals.tcl
# Verdi Automatic Signal Loading and Debug Setup
# Optimized for Arithmetic Unit and AI Chip Debug
# 
# ============================================================================
# USAGE: This script is for Verdi GUI only!
# ============================================================================
# 
# Method 1: Auto-load when opening Verdi
#   verdi -ssf work/arithmetic_unit.fsdb -play scripts/signals.tcl
#
# Method 2: Manual source in Verdi Tcl console
#   1. Open Verdi: make wave
#   2. Tools -> Tcl Console
#   3. source ../scripts/signals.tcl
#
# Method 3: Use Makefile helper (add to Makefile)
#   wave_auto:
#       verdi -ssf work/*.fsdb -play scripts/signals.tcl &
#
# NOTE: This script will NOT work in batch mode!
#       Verdi commands only work in Verdi GUI environment
#
# ============================================================================
...
puts "=========================================="
puts "  Verdi Signal Management Script"
puts "=========================================="
puts "Started at: [clock format $verdi_start_time -format {%Y-%m-%d %H:%M:%S}]"

# ============================================================================
# Verdi-compatible signal discovery and loading
# ============================================================================
proc auto_load_arithmetic_signals {} {
    puts "\n=== Auto-loading arithmetic unit signals ==="
    ...
    # Create signal group for arithmetic signals
        ...
        puts "Created group: Arithmetic_Core"
    }
    ...
    puts "Arithmetic signals loaded successfully"
}
...
proc auto_load_ai_signals {} {
    ...
    # Create AI processing group
        ...
        puts "Created group: AI_Processing"
    }
    ...
    puts "AI signals loaded successfully"
}
...
proc setup_memory_viewers {} {
    ...
    puts "Memory viewers setup complete"
}
...
proc create_waveform_groups {} {
    ...
    puts "Waveform groups created successfully"
}
...
proc setup_advanced_debug {} {
    puts "\n=== Setting up Verdi debug features ==="
    ...
    puts "Advanced debug features enabled"
}
...
proc setup_performance_analysis {} {
    puts "\n=== Setting up Verdi performance analysis ==="
    ...
    puts "Performance analysis setup complete"
}
...
proc list_available_signals {} {
        ...
        puts "\n=== Available Signals ==="
        puts "Total signals: [llength $all_signals]"
...
proc initialize_verdi_debug {} {
    puts "\n=== Initializing Verdi Debug Environment ==="
    ...
    # Configure waveform display
        ...
        puts "  Waveform display configured (ns, hex)"
    }
    
    puts "\n✓ Verdi debug environment initialized successfully"
}

# ============================================================================
# User-callable commands for interactive use
# ============================================================================
...
proc reload_signals {} {
    ...
    puts "✓ Signals reloaded"
}

# Show memory instances
proc show_memories {} {
    puts "\n=== Available Memory Instances ==="
    catch {
        ...
        if {[llength $mems] > 0} {
            ...
            puts "  No memory instances found"
        }
    }
}
...
# Add custom signal group
proc add_custom_group {group_name pattern_list} {
    puts "\n=== Creating custom group: $group_name ==="
    catch {
        addSignalGroup $group_name
        ...
        foreach pattern $pattern_list {
            ...
            puts "  Added signals matching: $pattern"
        }
        
        puts "✓ Custom group created"
    }
}
...
puts "\n=========================================="
puts "Starting automatic initialization..."
puts "=========================================="
...
puts "  add_custom_group <name> {patterns} - Create custom signal group"
puts ""
puts "Examples:"
puts "  reload_signals"
puts "  add_custom_group \"MyGroup\" {*clk* *data*}"
puts "  show_memories"
puts "=========================================="

set verdi_end_time [clock seconds]
set duration [expr $verdi_end_time - $verdi_start_time]
puts "\n✓ Verdi script loaded successfully in $duration seconds"
puts "=========================================="